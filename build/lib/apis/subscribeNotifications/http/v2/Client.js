...
class Client extends HttpClient_1.HttpClient {
    ...
    async getDescription() {
        ...
        logger.error('An unknown error occured.', withLogMetadata_1.withLogMetadata('api-client', 'subscribeNotifications', { error: data, status }));
    ...
    async getNotifications() {
        ...
        return stream_1.pipeline(data, jsonParser, heartbeatFilter, passThrough, (err) => {
            if (err) {
                ...
                logger.error('An error occured during stream piping.', withLogMetadata_1.withLogMetadata('api-client', 'subscribeNotifications', { err }));
            }
        });
    }
}
