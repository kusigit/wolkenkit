...
class PostgresPriorityQueueStore {
    ...
    static async create({ doesIdentifierMatchItem, expirationTime = 15000, hostName, port, userName, password, database, encryptConnection = false, tableNames }) {
    ...
    async getQueueIfLocked({ connection, discriminator, token }) {
        const queue = await this.getQueueByDiscriminator({ connection, discriminator });
        if (!queue) {
            throw new errors_1.errors.ItemNotFound(`Item for discriminator '${discriminator}' not found.`);
        }
        if (!queue.lock) {
            throw new errors_1.errors.ItemNotLocked(`Item for discriminator '${discriminator}' not locked.`);
        }
        if (queue.lock.token !== token) {
            throw new errors_1.errors.TokenMismatch(`Token mismatch for discriminator '${discriminator}'.`);
        }
        return queue;
    }
    ...
    async removeInternal({ connection, discriminator, itemIdentifier }) {
        const queue = await this.getQueueByDiscriminator({ connection, discriminator });
        if (!queue) {
            throw new errors_1.errors.ItemNotFound();
        }
        const { rows } = await connection.query({
