...
const processDomainEvent = async function ({ application, domainEventDispatcher, consumerProgressStore, lockStore, repository, issueCommand, performReplay }) {
    const { domainEvent, metadata } = await fetchDomainEvent_1.fetchDomainEvent({ domainEventDispatcher });
    const flowName = metadata.discriminator;
    logger.debug('Fetched and locked domain event for flow execution.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/flow', { itemIdentifier: domainEvent.getItemIdentifier(), metadata }));
    try {
        try {
        ...
        (async () => {
            await keepRenewingLock_1.keepRenewingLock({ flowName, flowPromise, domainEventDispatcher, token: metadata.token });
        })();
        const howToProceed = await flowPromise;
        switch (howToProceed) {
            case 'acknowledge': {
                await acknowledgeDomainEvent_1.acknowledgeDomainEvent({ flowName, token: metadata.token, domainEventDispatcher });
                logger.debug('Acknowledged domain event.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/flow', { itemIdentifier: domainEvent.getItemIdentifier(), metadata }));
                break;
            }
            case 'defer': {
                ...
                logger.debug('Skipped and deferred domain event.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/flow', { itemIdentifier: domainEvent.getItemIdentifier(), metadata }));
                break;
            }
            default: {
        ...
        logger.error('Failed to handle domain event.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/flow', { domainEvent, error: ex }));
        await acknowledgeDomainEvent_1.acknowledgeDomainEvent({ flowName, token: metadata.token, domainEventDispatcher });
    }
};
