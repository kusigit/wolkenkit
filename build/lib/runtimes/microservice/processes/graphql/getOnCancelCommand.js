...
const getOnCancelCommand = function ({ commandDispatcher }) {
    return async function ({ commandIdentifierWithClient }) {
        try {
            ...
            logger.debug('Cancelled command in command dispatcher.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/graphql', { commandIdentifierWithClient }));
        }
        catch (ex) {
            logger.error('Failed to cancel command in command dispatcher.', withLogMetadata_1.withLogMetadata('runtime', 'microservice/graphql', { commandIdentifierWithClient, error: ex }));
