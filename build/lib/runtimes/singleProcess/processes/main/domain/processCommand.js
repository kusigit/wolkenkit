...
const processCommand = async function ({ repository, priorityQueue, publishDomainEvents }) {
    const { command, metadata } = await fetchCommand_1.fetchCommand({ priorityQueue });
    logger.debug('Fetched and locked command for domain server.', withLogMetadata_1.withLogMetadata('runtime', 'singleProcess/main', { itemIdentifier: command.getItemIdentifier(), metadata }));
    try {
        try {
        ...
        (async () => {
            await keepRenewingLock_1.keepRenewingLock({ command, handleCommandPromise, priorityQueue, token: metadata.token });
        })();
        const domainEvents = await handleCommandPromise;
        await publishDomainEvents({ domainEvents });
    }
    catch (ex) {
        logger.error('Failed to handle command.', withLogMetadata_1.withLogMetadata('runtime', 'singleProcess/main', { command, error: ex }));
    }
    finally {
        await acknowledgeCommand_1.acknowledgeCommand({ command, token: metadata.token, priorityQueue });
        logger.debug('Processed and acknowledged command.', withLogMetadata_1.withLogMetadata('runtime', 'singleProcess/main', { itemIdentifier: command.getItemIdentifier(), metadata }));
    }
};
