...
const getTestsFor = function ({ createLockStore }) {
    ...
    suite('acquireLock', () => {
        ...
        test('throws an error if the lock is already in place.', async () => {
            ...
            await assertthat_1.assert.that(async () => {
                await lockStore.acquireLock({ value });
            }).is.throwingAsync((ex) => ex.code === errors_1.errors.LockAcquireFailed.code && ex.message === 'Failed to acquire lock.');
        });
        ...
        test('throws an error if the expiration date is in the past.', async () => {
            await assertthat_1.assert.that(async () => {
                await lockStore.acquireLock({ value, expiresAt: Date.now() - 100 });
            }).is.throwingAsync((ex) => ex.code === errors_1.errors.ExpirationInPast.code && ex.message === 'A lock must not expire in the past.');
        });
    });
    suite('isLocked', () => {
    ...
    suite('renewLock', () => {
        test('throws an error if the given lock does not exist.', async () => {
            ...
            }).is.throwingAsync((ex) => ex.code === errors_1.errors.LockRenewalFailed.code && ex.message === 'Failed to renew lock.');
        });
