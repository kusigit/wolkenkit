...
const getTestsFor = function ({ createDomainEventStore, teardownDomainEventStore }) {
    ...
    suite('getReplayForAggregate', function () {
        ...
        test('throws an error if the parameter fromRevision is less than 1.', async () => {
            await assertthat_1.assert.that(async () => await domainEventStore.getReplayForAggregate({ aggregateId: uuid_1.v4(), fromRevision: 0 })).is.throwingAsync((ex) => ex.code === errors_1.errors.ParameterInvalid.code && ex.message === `Parameter 'fromRevision' must be at least 1.`);
        });
        test('throws an error if the parameter toRevision is less than 1.', async () => {
            await assertthat_1.assert.that(async () => await domainEventStore.getReplayForAggregate({ aggregateId: uuid_1.v4(), toRevision: 0 })).is.throwingAsync((ex) => ex.code === errors_1.errors.ParameterInvalid.code && ex.message === `Parameter 'toRevision' must be at least 1.`);
        });
        test(`throws an error if the parameter 'fromRevision' is greater than 'toRevision'.`, async () => {
            await assertthat_1.assert.that(async () => await domainEventStore.getReplayForAggregate({ aggregateId: uuid_1.v4(), fromRevision: 5, toRevision: 3 })).is.throwingAsync((ex) => ex.code === errors_1.errors.ParameterInvalid.code && ex.message === `Parameter 'toRevision' must be greater or equal to 'fromRevision'.`);
        });
    });
    suite('storeDomainEvents', function () {
        ...
        test('throws an error if domain events is an empty array.', async () => {
            ...
            }).is.throwingAsync((ex) => ex.code === errors_1.errors.ParameterInvalid.code && ex.message === 'Domain events are missing.');
        });
        ...
        test('throws an error if the aggregate id and revision of the new domain event are already in use.', async () => {
            ...
            await assertthat_1.assert.that(async () => {
                await domainEventStore.storeDomainEvents({ domainEvents: [domainEvent] });
            }).is.throwingAsync((ex) => ex.code === errors_1.errors.RevisionAlreadyExists.code && ex.message === 'Aggregate id and revision already exist.');
        });
    ...
    suite('getReplay', function () {
        ...
        test('throws an error if the parameter fromTimestamp is less than 0.', async () => {
            await assertthat_1.assert.that(async () => await domainEventStore.getReplay({ fromTimestamp: -1 })).is.throwingAsync((ex) => ex.code === errors_1.errors.ParameterInvalid.code && ex.message === `Parameter 'fromTimestamp' must be at least 0.`);
        });
    });
